...
export async function getTopAlbums(accessToken, user, limit = 10) {
    const url = `${process.env.LASTFM_BASE_URL}?method=user.gettopalbums&user=${user}&api_key=${accessToken}&format=json&limit=${limit}`;
    try {
